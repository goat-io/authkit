...
`OrganizationSSO` is currently configured by the application operator at startup; it is not yet a self-service organization-admin API. The issuer must be HTTPS and serve standard OIDC discovery. Plain OAuth 2.0 without OIDC identity tokens and SAML are separate integrations. For a self-service admin UI, keep client secrets encrypted and validate issuer URLs before allowing an organization to register a connection.
